...
class Provsioning_Group_Assignment_Request_ReferencesType extends AbstractStructBase
{
     ...
     * @param \WorkdayWsdl\\StructType\Unique_IdentifierObjectType[] $provisioning_Group_Assignment_Request_Reference
     */
    ...
    public static function validateProvisioning_Group_Assignment_Request_ReferenceForArrayConstraintsFromSetProvisioning_Group_Assignment_Request_Reference(array $values = array())
    {
        ...
        foreach ($values as $provsioning_Group_Assignment_Request_ReferencesTypeProvisioning_Group_Assignment_Request_ReferenceItem) {
            ...
            if (!$provsioning_Group_Assignment_Request_ReferencesTypeProvisioning_Group_Assignment_Request_ReferenceItem instanceof \WorkdayWsdl\\StructType\Unique_IdentifierObjectType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Provisioning_Group_Assignment_Request_Reference property can only contain items of type \WorkdayWsdl\\StructType\Unique_IdentifierObjectType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @param \WorkdayWsdl\\StructType\Unique_IdentifierObjectType[] $provisioning_Group_Assignment_Request_Reference
     * @return \WorkdayWsdl\\StructType\Provsioning_Group_Assignment_Request_ReferencesType
     */
     ...
     * @return \WorkdayWsdl\\StructType\Provsioning_Group_Assignment_Request_ReferencesType
     */
    public function addToProvisioning_Group_Assignment_Request_Reference(\WorkdayWsdl\\StructType\Unique_IdentifierObjectType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \WorkdayWsdl\\StructType\Unique_IdentifierObjectType) {
            throw new \InvalidArgumentException(sprintf('The Provisioning_Group_Assignment_Request_Reference property can only contain items of type \WorkdayWsdl\\StructType\Unique_IdentifierObjectType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
