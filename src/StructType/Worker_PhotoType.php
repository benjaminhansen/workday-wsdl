...
class Worker_PhotoType extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \WorkdayWsdl\\StructType\Employee_Photo_DataType[]
     */
     ...
     * @param \WorkdayWsdl\\StructType\Employee_Photo_DataType[] $worker_Photo_Data
     */
    public function __construct(\WorkdayWsdl\\StructType\WorkerObjectType $worker_Reference = null, array $worker_Photo_Data = array())
    {
        $this
     ...
     * @param \WorkdayWsdl\\StructType\WorkerObjectType $worker_Reference
     * @return \WorkdayWsdl\\StructType\Worker_PhotoType
     */
     ...
     * @return \WorkdayWsdl\\StructType\Employee_Photo_DataType[]|null
     */
    ...
    public static function validateWorker_Photo_DataForArrayConstraintsFromSetWorker_Photo_Data(array $values = array())
    {
        ...
        foreach ($values as $worker_PhotoTypeWorker_Photo_DataItem) {
            ...
            if (!$worker_PhotoTypeWorker_Photo_DataItem instanceof \WorkdayWsdl\\StructType\Employee_Photo_DataType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Worker_Photo_Data property can only contain items of type \WorkdayWsdl\\StructType\Employee_Photo_DataType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @throws \InvalidArgumentException
     * @param \WorkdayWsdl\\StructType\Employee_Photo_DataType $item
     * @return \WorkdayWsdl\\StructType\Worker_PhotoType
     */
    public function addToWorker_Photo_Data(\WorkdayWsdl\\StructType\Employee_Photo_DataType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \WorkdayWsdl\\StructType\Employee_Photo_DataType) {
            throw new \InvalidArgumentException(sprintf('The Worker_Photo_Data property can only contain items of type \WorkdayWsdl\\StructType\Employee_Photo_DataType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
