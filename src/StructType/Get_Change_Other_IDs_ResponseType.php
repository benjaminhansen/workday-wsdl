...
class Get_Change_Other_IDs_ResponseType extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \StructType\Get_Change_Other_IDs_Request_ReferencesType[]
     */
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \StructType\Change_Other_IDs_Response_DataType[]
     */
     ...
     * @param \StructType\Get_Change_Other_IDs_Request_ReferencesType[] $request_References
     * @param \StructType\Response_FilterType[] $response_Filter
     * @param \StructType\Response_ResultsType[] $response_Results
     * @param \StructType\Change_Other_IDs_Response_DataType[] $response_Data
     ...
     * @return \StructType\Get_Change_Other_IDs_Request_ReferencesType[]|null
     */
    ...
    public static function validateRequest_ReferencesForArrayConstraintsFromSetRequest_References(array $values = array())
    {
        ...
        foreach ($values as $get_Change_Other_IDs_ResponseTypeRequest_ReferencesItem) {
            ...
            if (!$get_Change_Other_IDs_ResponseTypeRequest_ReferencesItem instanceof \StructType\Get_Change_Other_IDs_Request_ReferencesType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Request_References property can only contain items of type \StructType\Get_Change_Other_IDs_Request_ReferencesType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @param \StructType\Get_Change_Other_IDs_Request_ReferencesType[] $request_References
     * @return \StructType\Get_Change_Other_IDs_ResponseType
     */
     ...
     * @throws \InvalidArgumentException
     * @param \StructType\Get_Change_Other_IDs_Request_ReferencesType $item
     * @return \StructType\Get_Change_Other_IDs_ResponseType
     */
    public function addToRequest_References(\StructType\Get_Change_Other_IDs_Request_ReferencesType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\Get_Change_Other_IDs_Request_ReferencesType) {
            throw new \InvalidArgumentException(sprintf('The Request_References property can only contain items of type \StructType\Get_Change_Other_IDs_Request_ReferencesType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
    ...
    public static function validateResponse_FilterForArrayConstraintsFromSetResponse_Filter(array $values = array())
    {
        ...
        foreach ($values as $get_Change_Other_IDs_ResponseTypeResponse_FilterItem) {
            // validation for constraint: itemType
            if (!$get_Change_Other_IDs_ResponseTypeResponse_FilterItem instanceof \StructType\Response_FilterType) {
    ...
    public static function validateResponse_ResultsForArrayConstraintsFromSetResponse_Results(array $values = array())
    {
        ...
        foreach ($values as $get_Change_Other_IDs_ResponseTypeResponse_ResultsItem) {
            // validation for constraint: itemType
            if (!$get_Change_Other_IDs_ResponseTypeResponse_ResultsItem instanceof \StructType\Response_ResultsType) {
     ...
     * @return \StructType\Change_Other_IDs_Response_DataType[]|null
     */
    ...
    public static function validateResponse_DataForArrayConstraintsFromSetResponse_Data(array $values = array())
    {
        ...
        foreach ($values as $get_Change_Other_IDs_ResponseTypeResponse_DataItem) {
            // validation for constraint: itemType
            if (!$get_Change_Other_IDs_ResponseTypeResponse_DataItem instanceof \StructType\Change_Other_IDs_Response_DataType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Response_Data property can only contain items of type \StructType\Change_Other_IDs_Response_DataType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @throws \InvalidArgumentException
     * @param \StructType\Change_Other_IDs_Response_DataType $item
     * @return \StructType\Get_Change_Other_IDs_ResponseType
     */
    public function addToResponse_Data(\StructType\Change_Other_IDs_Response_DataType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\Change_Other_IDs_Response_DataType) {
            throw new \InvalidArgumentException(sprintf('The Response_Data property can only contain items of type \StructType\Change_Other_IDs_Response_DataType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
