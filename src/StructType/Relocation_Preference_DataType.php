...
class Relocation_Preference_DataType extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \StructType\Relocation_AreaObjectType[]
     */
     ...
     * @param \StructType\Common_Boolean_EnumerationObjectType $short_Term_Relocation_Reference
     * @param \StructType\Relocation_AreaObjectType[] $short_Term_Relocation_Area__Reference
     * @param \StructType\Common_Boolean_EnumerationObjectType $long_Term_Relocation_Reference
     * @param \StructType\Relocation_AreaObjectType[] $long_Term_Relocation_Area__Reference
     * @param string $additional_Information
     */
    public function __construct(\StructType\Common_Boolean_EnumerationObjectType $short_Term_Relocation_Reference = null, array $short_Term_Relocation_Area__Reference = array(), \StructType\Common_Boolean_EnumerationObjectType $long_Term_Relocation_Reference = null, array $long_Term_Relocation_Area__Reference = array(), $additional_Information = null)
    {
        $this
     ...
     * @param \StructType\Common_Boolean_EnumerationObjectType $short_Term_Relocation_Reference
     * @return \StructType\Relocation_Preference_DataType
     */
    public function setShort_Term_Relocation_Reference(\StructType\Common_Boolean_EnumerationObjectType $short_Term_Relocation_Reference = null)
    {
     ...
     * @return \StructType\Relocation_AreaObjectType[]|null
     */
    ...
    public static function validateShort_Term_Relocation_Area__ReferenceForArrayConstraintsFromSetShort_Term_Relocation_Area__Reference(array $values = array())
    {
        ...
        foreach ($values as $relocation_Preference_DataTypeShort_Term_Relocation_Area__ReferenceItem) {
            ...
            if (!$relocation_Preference_DataTypeShort_Term_Relocation_Area__ReferenceItem instanceof \StructType\Relocation_AreaObjectType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Short_Term_Relocation_Area__Reference property can only contain items of type \StructType\Relocation_AreaObjectType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @throws \InvalidArgumentException
     * @param \StructType\Relocation_AreaObjectType $item
     * @return \StructType\Relocation_Preference_DataType
     */
    public function addToShort_Term_Relocation_Area__Reference(\StructType\Relocation_AreaObjectType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\Relocation_AreaObjectType) {
            throw new \InvalidArgumentException(sprintf('The Short_Term_Relocation_Area__Reference property can only contain items of type \StructType\Relocation_AreaObjectType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
     ...
     * @return \StructType\Relocation_Preference_DataType
     */
    public function setLong_Term_Relocation_Reference(\StructType\Common_Boolean_EnumerationObjectType $long_Term_Relocation_Reference = null)
    {
    ...
    public static function validateLong_Term_Relocation_Area__ReferenceForArrayConstraintsFromSetLong_Term_Relocation_Area__Reference(array $values = array())
    {
        ...
        foreach ($values as $relocation_Preference_DataTypeLong_Term_Relocation_Area__ReferenceItem) {
            // validation for constraint: itemType
            if (!$relocation_Preference_DataTypeLong_Term_Relocation_Area__ReferenceItem instanceof \StructType\Relocation_AreaObjectType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Long_Term_Relocation_Area__Reference property can only contain items of type \StructType\Relocation_AreaObjectType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @return \StructType\Relocation_Preference_DataType
     */
    public function addToLong_Term_Relocation_Area__Reference(\StructType\Relocation_AreaObjectType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\Relocation_AreaObjectType) {
            throw new \InvalidArgumentException(sprintf('The Long_Term_Relocation_Area__Reference property can only contain items of type \StructType\Relocation_AreaObjectType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
