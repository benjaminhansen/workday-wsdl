...
class Job_Family_Request_ReferencesType extends AbstractStructBase
{
    ...
    public static function validateJob_Family_ReferenceForArrayConstraintsFromSetJob_Family_Reference(array $values = array())
    {
        ...
        foreach ($values as $job_Family_Request_ReferencesTypeJob_Family_ReferenceItem) {
            ...
            if (!$job_Family_Request_ReferencesTypeJob_Family_ReferenceItem instanceof \WorkdayWsdl\\StructType\Job_FamilyObjectType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Job_Family_Reference property can only contain items of type \WorkdayWsdl\\StructType\Job_FamilyObjectType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @param \WorkdayWsdl\\StructType\Job_FamilyObjectType[] $job_Family_Reference
     * @return \WorkdayWsdl\\StructType\Job_Family_Request_ReferencesType
     */
     ...
     * @return \WorkdayWsdl\\StructType\Job_Family_Request_ReferencesType
     */
    public function addToJob_Family_Reference(\WorkdayWsdl\\StructType\Job_FamilyObjectType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \WorkdayWsdl\\StructType\Job_FamilyObjectType) {
            throw new \InvalidArgumentException(sprintf('The Job_Family_Reference property can only contain items of type \WorkdayWsdl\\StructType\Job_FamilyObjectType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
