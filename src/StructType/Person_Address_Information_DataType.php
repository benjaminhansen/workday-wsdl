...
class Person_Address_Information_DataType extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \WorkdayWsdl\\StructType\Person_Address_DataType[]
     */
     ...
     * @param \WorkdayWsdl\\StructType\Person_Address_DataType[] $address_Information_Data
     ...
     * @return \WorkdayWsdl\\StructType\Person_Address_DataType[]|null
     */
    ...
    public static function validateAddress_Information_DataForArrayConstraintsFromSetAddress_Information_Data(array $values = array())
    {
        ...
        foreach ($values as $person_Address_Information_DataTypeAddress_Information_DataItem) {
            ...
            if (!$person_Address_Information_DataTypeAddress_Information_DataItem instanceof \WorkdayWsdl\\StructType\Person_Address_DataType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Address_Information_Data property can only contain items of type \WorkdayWsdl\\StructType\Person_Address_DataType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @param \WorkdayWsdl\\StructType\Person_Address_DataType[] $address_Information_Data
     * @return \WorkdayWsdl\\StructType\Person_Address_Information_DataType
     */
     ...
     * @throws \InvalidArgumentException
     * @param \WorkdayWsdl\\StructType\Person_Address_DataType $item
     * @return \WorkdayWsdl\\StructType\Person_Address_Information_DataType
     */
    public function addToAddress_Information_Data(\WorkdayWsdl\\StructType\Person_Address_DataType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \WorkdayWsdl\\StructType\Person_Address_DataType) {
            throw new \InvalidArgumentException(sprintf('The Address_Information_Data property can only contain items of type \WorkdayWsdl\\StructType\Person_Address_DataType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
