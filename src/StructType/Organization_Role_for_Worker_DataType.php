...
class Organization_Role_for_Worker_DataType extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \WorkdayWsdl\\StructType\Organization_Role_Assignment_for_Worker_DataType[]
     */
     ...
     * @param \WorkdayWsdl\\StructType\Organization_Role_Assignment_for_Worker_DataType[] $organization_Role_Data
     */
     ...
     * @param \WorkdayWsdl\\StructType\Assignable_RoleObjectType $organization_Role_Reference
     * @return \WorkdayWsdl\\StructType\Organization_Role_for_Worker_DataType
     */
     ...
     * @return \WorkdayWsdl\\StructType\Organization_Role_Assignment_for_Worker_DataType[]|null
     */
    ...
    public static function validateOrganization_Role_DataForArrayConstraintsFromSetOrganization_Role_Data(array $values = array())
    {
        ...
        foreach ($values as $organization_Role_for_Worker_DataTypeOrganization_Role_DataItem) {
            ...
            if (!$organization_Role_for_Worker_DataTypeOrganization_Role_DataItem instanceof \WorkdayWsdl\\StructType\Organization_Role_Assignment_for_Worker_DataType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Organization_Role_Data property can only contain items of type \WorkdayWsdl\\StructType\Organization_Role_Assignment_for_Worker_DataType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @throws \InvalidArgumentException
     * @param \WorkdayWsdl\\StructType\Organization_Role_Assignment_for_Worker_DataType $item
     * @return \WorkdayWsdl\\StructType\Organization_Role_for_Worker_DataType
     */
    public function addToOrganization_Role_Data(\WorkdayWsdl\\StructType\Organization_Role_Assignment_for_Worker_DataType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \WorkdayWsdl\\StructType\Organization_Role_Assignment_for_Worker_DataType) {
            throw new \InvalidArgumentException(sprintf('The Organization_Role_Data property can only contain items of type \WorkdayWsdl\\StructType\Organization_Role_Assignment_for_Worker_DataType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
