...
class Difficulty_to_Fill_Request_ReferencesType extends AbstractStructBase
{
    ...
    public static function validateDifficulty_to_Fill_ReferenceForArrayConstraintsFromSetDifficulty_to_Fill_Reference(array $values = array())
    {
        ...
        foreach ($values as $difficulty_to_Fill_Request_ReferencesTypeDifficulty_to_Fill_ReferenceItem) {
            ...
            if (!$difficulty_to_Fill_Request_ReferencesTypeDifficulty_to_Fill_ReferenceItem instanceof \WorkdayWsdl\\StructType\Difficulty_to_FillObjectType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Difficulty_to_Fill_Reference property can only contain items of type \WorkdayWsdl\\StructType\Difficulty_to_FillObjectType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @param \WorkdayWsdl\\StructType\Difficulty_to_FillObjectType[] $difficulty_to_Fill_Reference
     * @return \WorkdayWsdl\\StructType\Difficulty_to_Fill_Request_ReferencesType
     */
     ...
     * @throws \InvalidArgumentException
     * @param \WorkdayWsdl\\StructType\Difficulty_to_FillObjectType $item
     * @return \WorkdayWsdl\\StructType\Difficulty_to_Fill_Request_ReferencesType
     */
    public function addToDifficulty_to_Fill_Reference(\WorkdayWsdl\\StructType\Difficulty_to_FillObjectType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \WorkdayWsdl\\StructType\Difficulty_to_FillObjectType) {
            throw new \InvalidArgumentException(sprintf('The Difficulty_to_Fill_Reference property can only contain items of type \WorkdayWsdl\\StructType\Difficulty_to_FillObjectType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
