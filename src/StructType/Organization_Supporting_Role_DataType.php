...
class Organization_Supporting_Role_DataType extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \StructType\Worker_Organization_Role_Assignment_DataType[]
     */
     ...
     * @param \StructType\Worker_Organization_Role_Assignment_DataType[] $organization_Role_Data
     */
     ...
     * @param \StructType\Assignable_RoleObjectType $organization_Role_Reference
     * @return \StructType\Organization_Supporting_Role_DataType
     */
     ...
     * @return \StructType\Worker_Organization_Role_Assignment_DataType[]|null
     */
    ...
    public static function validateOrganization_Role_DataForArrayConstraintsFromSetOrganization_Role_Data(array $values = array())
    {
        ...
        foreach ($values as $organization_Supporting_Role_DataTypeOrganization_Role_DataItem) {
            ...
            if (!$organization_Supporting_Role_DataTypeOrganization_Role_DataItem instanceof \StructType\Worker_Organization_Role_Assignment_DataType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Organization_Role_Data property can only contain items of type \StructType\Worker_Organization_Role_Assignment_DataType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @throws \InvalidArgumentException
     * @param \StructType\Worker_Organization_Role_Assignment_DataType $item
     * @return \StructType\Organization_Supporting_Role_DataType
     */
    public function addToOrganization_Role_Data(\StructType\Worker_Organization_Role_Assignment_DataType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\Worker_Organization_Role_Assignment_DataType) {
            throw new \InvalidArgumentException(sprintf('The Organization_Role_Data property can only contain items of type \StructType\Worker_Organization_Role_Assignment_DataType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
