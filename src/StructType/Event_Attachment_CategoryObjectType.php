...
class Event_Attachment_CategoryObjectType extends AbstractStructBase
{
    ...
    public static function validateIDForArrayConstraintsFromSetID(array $values = array())
    {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The ID property can only contain items of type \StructType\Event_Attachment_CategoryObjectIDType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
    ...
    public function addToID(\StructType\Event_Attachment_CategoryObjectIDType $item)
    {
        ...
        if (!$item instanceof \StructType\Event_Attachment_CategoryObjectIDType) {
            throw new \InvalidArgumentException(sprintf('The ID property can only contain items of type \StructType\Event_Attachment_CategoryObjectIDType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->ID[] = $item;
        return $this;
    }
