...
class Committee_Meeting_Request_ReferencesType extends AbstractStructBase
{
    ...
    public static function validateCommittee_Meeting_ReferenceForArrayConstraintsFromSetCommittee_Meeting_Reference(array $values = array())
    {
        ...
        foreach ($values as $committee_Meeting_Request_ReferencesTypeCommittee_Meeting_ReferenceItem) {
            ...
            if (!$committee_Meeting_Request_ReferencesTypeCommittee_Meeting_ReferenceItem instanceof \WorkdayWsdl\\StructType\Committee_MeetingObjectType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Committee_Meeting_Reference property can only contain items of type \WorkdayWsdl\\StructType\Committee_MeetingObjectType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @param \WorkdayWsdl\\StructType\Committee_MeetingObjectType[] $committee_Meeting_Reference
     * @return \WorkdayWsdl\\StructType\Committee_Meeting_Request_ReferencesType
     */
     ...
     * @throws \InvalidArgumentException
     * @param \WorkdayWsdl\\StructType\Committee_MeetingObjectType $item
     * @return \WorkdayWsdl\\StructType\Committee_Meeting_Request_ReferencesType
     */
    public function addToCommittee_Meeting_Reference(\WorkdayWsdl\\StructType\Committee_MeetingObjectType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \WorkdayWsdl\\StructType\Committee_MeetingObjectType) {
            throw new \InvalidArgumentException(sprintf('The Committee_Meeting_Reference property can only contain items of type \WorkdayWsdl\\StructType\Committee_MeetingObjectType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
