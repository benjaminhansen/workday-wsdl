...
class Change_Licenses_Response_DataType extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \WorkdayWsdl\\StructType\Change_Licenses_Response_WrapperType[]
     */
     ...
     * @param \WorkdayWsdl\\StructType\Change_Licenses_Response_WrapperType[] $change_Licenses
     */
     ...
     * @return \WorkdayWsdl\\StructType\Change_Licenses_Response_WrapperType[]|null
     */
    ...
    public static function validateChange_LicensesForArrayConstraintsFromSetChange_Licenses(array $values = array())
    {
        ...
        foreach ($values as $change_Licenses_Response_DataTypeChange_LicensesItem) {
            ...
            if (!$change_Licenses_Response_DataTypeChange_LicensesItem instanceof \WorkdayWsdl\\StructType\Change_Licenses_Response_WrapperType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Change_Licenses property can only contain items of type \WorkdayWsdl\\StructType\Change_Licenses_Response_WrapperType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @param \WorkdayWsdl\\StructType\Change_Licenses_Response_WrapperType[] $change_Licenses
     * @return \WorkdayWsdl\\StructType\Change_Licenses_Response_DataType
     */
     ...
     * @throws \InvalidArgumentException
     * @param \WorkdayWsdl\\StructType\Change_Licenses_Response_WrapperType $item
     * @return \WorkdayWsdl\\StructType\Change_Licenses_Response_DataType
     */
    public function addToChange_Licenses(\WorkdayWsdl\\StructType\Change_Licenses_Response_WrapperType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \WorkdayWsdl\\StructType\Change_Licenses_Response_WrapperType) {
            throw new \InvalidArgumentException(sprintf('The Change_Licenses property can only contain items of type \WorkdayWsdl\\StructType\Change_Licenses_Response_WrapperType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
