...
/**
 * This class stands for Get_Worker_Photos_ResponseType StructType
 * Meta information extracted from the WSDL
 * - documentation: Element containing Get Worker Photo response elements including received criteria, references, and response data.
 * @subpackage Structs
 */
class Get_Worker_Photos_ResponseType extends AbstractStructBase
{
    /**
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \WorkdayWsdl\\StructType\Worker_Photos_Response_DataType[]
     */
    ...
    public $version;
    /**
     * Constructor method for Get_Worker_Photos_ResponseType
     * @uses Get_Worker_Photos_ResponseType::setRequest_References()
     * @uses Get_Worker_Photos_ResponseType::setResponse_Filter()
     * @uses Get_Worker_Photos_ResponseType::setResponse_Results()
     * @uses Get_Worker_Photos_ResponseType::setResponse_Data()
     * @uses Get_Worker_Photos_ResponseType::setVersion()
     ...
     * @param \WorkdayWsdl\\StructType\Worker_Photos_Response_DataType[] $response_Data
    ...
    public static function validateRequest_ReferencesForArrayConstraintsFromSetRequest_References(array $values = array())
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $get_Worker_Photos_ResponseTypeRequest_ReferencesItem) {
            // validation for constraint: itemType
            if (!$get_Worker_Photos_ResponseTypeRequest_ReferencesItem instanceof \WorkdayWsdl\\StructType\Worker_Request_ReferencesType) {
                $invalidValues[] = is_object($get_Worker_Photos_ResponseTypeRequest_ReferencesItem) ? get_class($get_Worker_Photos_ResponseTypeRequest_ReferencesItem) : sprintf('%s(%s)', gettype($get_Worker_Photos_ResponseTypeRequest_ReferencesItem), var_export($get_Worker_Photos_ResponseTypeRequest_ReferencesItem, true));
            }
        }
     ...
     * @param \WorkdayWsdl\\StructType\Worker_Request_ReferencesType[] $request_References
     * @return \WorkdayWsdl\\StructType\Get_Worker_Photos_ResponseType
     */
    ...
    public static function validateResponse_FilterForArrayConstraintsFromSetResponse_Filter(array $values = array())
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $get_Worker_Photos_ResponseTypeResponse_FilterItem) {
            // validation for constraint: itemType
            if (!$get_Worker_Photos_ResponseTypeResponse_FilterItem instanceof \WorkdayWsdl\\StructType\Response_FilterType) {
                $invalidValues[] = is_object($get_Worker_Photos_ResponseTypeResponse_FilterItem) ? get_class($get_Worker_Photos_ResponseTypeResponse_FilterItem) : sprintf('%s(%s)', gettype($get_Worker_Photos_ResponseTypeResponse_FilterItem), var_export($get_Worker_Photos_ResponseTypeResponse_FilterItem, true));
            }
        }
        if (!empty($invalidValues)) {
    ...
    public static function validateResponse_ResultsForArrayConstraintsFromSetResponse_Results(array $values = array())
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $get_Worker_Photos_ResponseTypeResponse_ResultsItem) {
            // validation for constraint: itemType
            if (!$get_Worker_Photos_ResponseTypeResponse_ResultsItem instanceof \WorkdayWsdl\\StructType\Response_ResultsType) {
                $invalidValues[] = is_object($get_Worker_Photos_ResponseTypeResponse_ResultsItem) ? get_class($get_Worker_Photos_ResponseTypeResponse_ResultsItem) : sprintf('%s(%s)', gettype($get_Worker_Photos_ResponseTypeResponse_ResultsItem), var_export($get_Worker_Photos_ResponseTypeResponse_ResultsItem, true));
            }
        }
        if (!empty($invalidValues)) {
     ...
     * @return \WorkdayWsdl\\StructType\Worker_Photos_Response_DataType[]|null
     */
    ...
    public static function validateResponse_DataForArrayConstraintsFromSetResponse_Data(array $values = array())
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $get_Worker_Photos_ResponseTypeResponse_DataItem) {
            // validation for constraint: itemType
            if (!$get_Worker_Photos_ResponseTypeResponse_DataItem instanceof \WorkdayWsdl\\StructType\Worker_Photos_Response_DataType) {
                $invalidValues[] = is_object($get_Worker_Photos_ResponseTypeResponse_DataItem) ? get_class($get_Worker_Photos_ResponseTypeResponse_DataItem) : sprintf('%s(%s)', gettype($get_Worker_Photos_ResponseTypeResponse_DataItem), var_export($get_Worker_Photos_ResponseTypeResponse_DataItem, true));
            }
        }
        if (!empty($invalidValues)) {
            $message = sprintf('The Response_Data property can only contain items of type \WorkdayWsdl\\StructType\Worker_Photos_Response_DataType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @throws \InvalidArgumentException
     * @param \WorkdayWsdl\\StructType\Worker_Photos_Response_DataType $item
     * @return \WorkdayWsdl\\StructType\Get_Worker_Photos_ResponseType
     */
    public function addToResponse_Data(\WorkdayWsdl\\StructType\Worker_Photos_Response_DataType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \WorkdayWsdl\\StructType\Worker_Photos_Response_DataType) {
            throw new \InvalidArgumentException(sprintf('The Response_Data property can only contain items of type \WorkdayWsdl\\StructType\Worker_Photos_Response_DataType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
