...
class Workers_Compensation_Code_Response_DataType extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \WorkdayWsdl\\StructType\Workers_Compensation_CodeType[]
     */
     ...
     * @param \WorkdayWsdl\\StructType\Workers_Compensation_CodeType[] $workers_Compensation_Code
     */
     ...
     * @return \WorkdayWsdl\\StructType\Workers_Compensation_CodeType[]|null
     */
    ...
    public static function validateWorkers_Compensation_CodeForArrayConstraintsFromSetWorkers_Compensation_Code(array $values = array())
    {
        ...
        foreach ($values as $workers_Compensation_Code_Response_DataTypeWorkers_Compensation_CodeItem) {
            ...
            if (!$workers_Compensation_Code_Response_DataTypeWorkers_Compensation_CodeItem instanceof \WorkdayWsdl\\StructType\Workers_Compensation_CodeType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Workers_Compensation_Code property can only contain items of type \WorkdayWsdl\\StructType\Workers_Compensation_CodeType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @param \WorkdayWsdl\\StructType\Workers_Compensation_CodeType[] $workers_Compensation_Code
     * @return \WorkdayWsdl\\StructType\Workers_Compensation_Code_Response_DataType
     */
     ...
     * @throws \InvalidArgumentException
     * @param \WorkdayWsdl\\StructType\Workers_Compensation_CodeType $item
     * @return \WorkdayWsdl\\StructType\Workers_Compensation_Code_Response_DataType
     */
    public function addToWorkers_Compensation_Code(\WorkdayWsdl\\StructType\Workers_Compensation_CodeType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \WorkdayWsdl\\StructType\Workers_Compensation_CodeType) {
            throw new \InvalidArgumentException(sprintf('The Workers_Compensation_Code property can only contain items of type \WorkdayWsdl\\StructType\Workers_Compensation_CodeType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
