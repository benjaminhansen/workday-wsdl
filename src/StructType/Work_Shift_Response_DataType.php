...
class Work_Shift_Response_DataType extends AbstractStructBase
{
     ...
     * @return \WorkdayWsdl\\StructType\Work_ShiftType[]|null
     */
    ...
    public static function validateWork_ShiftForArrayConstraintsFromSetWork_Shift(array $values = array())
    {
        ...
        foreach ($values as $work_Shift_Response_DataTypeWork_ShiftItem) {
            ...
            if (!$work_Shift_Response_DataTypeWork_ShiftItem instanceof \WorkdayWsdl\\StructType\Work_ShiftType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Work_Shift property can only contain items of type \WorkdayWsdl\\StructType\Work_ShiftType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @param \WorkdayWsdl\\StructType\Work_ShiftType[] $work_Shift
     * @return \WorkdayWsdl\\StructType\Work_Shift_Response_DataType
     */
     ...
     * @throws \InvalidArgumentException
     * @param \WorkdayWsdl\\StructType\Work_ShiftType $item
     * @return \WorkdayWsdl\\StructType\Work_Shift_Response_DataType
     */
    public function addToWork_Shift(\WorkdayWsdl\\StructType\Work_ShiftType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \WorkdayWsdl\\StructType\Work_ShiftType) {
            throw new \InvalidArgumentException(sprintf('The Work_Shift property can only contain items of type \WorkdayWsdl\\StructType\Work_ShiftType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
