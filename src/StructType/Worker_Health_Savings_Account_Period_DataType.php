...
class Worker_Health_Savings_Account_Period_DataType extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \WorkdayWsdl\\StructType\Health_Savings_Account_Coverage_DataType[]
     */
     ...
     * @param \WorkdayWsdl\\StructType\Health_Savings_Account_Coverage_DataType[] $health_Savings_Account_Coverage_Data
     */
    public function __construct(\WorkdayWsdl\\StructType\Enrollment_Period_DataType $enrollment_Period_Data = null, array $health_Savings_Account_Coverage_Data = array())
    {
        $this
     ...
     * @param \WorkdayWsdl\\StructType\Enrollment_Period_DataType $enrollment_Period_Data
     * @return \WorkdayWsdl\\StructType\Worker_Health_Savings_Account_Period_DataType
     */
     ...
     * @return \WorkdayWsdl\\StructType\Health_Savings_Account_Coverage_DataType[]|null
     */
    ...
    public static function validateHealth_Savings_Account_Coverage_DataForArrayConstraintsFromSetHealth_Savings_Account_Coverage_Data(array $values = array())
    {
        ...
        foreach ($values as $worker_Health_Savings_Account_Period_DataTypeHealth_Savings_Account_Coverage_DataItem) {
            ...
            if (!$worker_Health_Savings_Account_Period_DataTypeHealth_Savings_Account_Coverage_DataItem instanceof \WorkdayWsdl\\StructType\Health_Savings_Account_Coverage_DataType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Health_Savings_Account_Coverage_Data property can only contain items of type \WorkdayWsdl\\StructType\Health_Savings_Account_Coverage_DataType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @throws \InvalidArgumentException
     * @param \WorkdayWsdl\\StructType\Health_Savings_Account_Coverage_DataType $item
     * @return \WorkdayWsdl\\StructType\Worker_Health_Savings_Account_Period_DataType
     */
    public function addToHealth_Savings_Account_Coverage_Data(\WorkdayWsdl\\StructType\Health_Savings_Account_Coverage_DataType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \WorkdayWsdl\\StructType\Health_Savings_Account_Coverage_DataType) {
            throw new \InvalidArgumentException(sprintf('The Health_Savings_Account_Coverage_Data property can only contain items of type \WorkdayWsdl\\StructType\Health_Savings_Account_Coverage_DataType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
