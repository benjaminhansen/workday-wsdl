...
class Change_Work_Contact_Information_Response_WrapperType extends AbstractStructBase
{
     ...
     */
    public function __construct(\StructType\RoleObjectType $person_Reference = null, array $change_Work_Contact_Information_Data = array())
    {
        $this
     ...
     * @param \StructType\RoleObjectType $person_Reference
     * @return \StructType\Change_Work_Contact_Information_Response_WrapperType
     */
    ...
    public static function validateChange_Work_Contact_Information_DataForArrayConstraintsFromSetChange_Work_Contact_Information_Data(array $values = array())
    {
        ...
        foreach ($values as $change_Work_Contact_Information_Response_WrapperTypeChange_Work_Contact_Information_DataItem) {
            ...
            if (!$change_Work_Contact_Information_Response_WrapperTypeChange_Work_Contact_Information_DataItem instanceof \StructType\Change_Work_Contact_Information_Business_Process_DataType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Change_Work_Contact_Information_Data property can only contain items of type \StructType\Change_Work_Contact_Information_Business_Process_DataType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @throws \InvalidArgumentException
     * @param \StructType\Change_Work_Contact_Information_Business_Process_DataType $item
     * @return \StructType\Change_Work_Contact_Information_Response_WrapperType
     */
    public function addToChange_Work_Contact_Information_Data(\StructType\Change_Work_Contact_Information_Business_Process_DataType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\Change_Work_Contact_Information_Business_Process_DataType) {
            throw new \InvalidArgumentException(sprintf('The Change_Work_Contact_Information_Data property can only contain items of type \StructType\Change_Work_Contact_Information_Business_Process_DataType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
