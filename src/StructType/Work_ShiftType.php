...
class Work_ShiftType extends AbstractStructBase
{
     ...
     */
    public function __construct(\WorkdayWsdl\\StructType\Work_ShiftObjectType $work_Shift_Reference = null, array $work_Shift_Data = array())
    {
        $this
     ...
     * @param \WorkdayWsdl\\StructType\Work_ShiftObjectType $work_Shift_Reference
     * @return \WorkdayWsdl\\StructType\Work_ShiftType
     */
    ...
    public static function validateWork_Shift_DataForArrayConstraintsFromSetWork_Shift_Data(array $values = array())
    {
        ...
        foreach ($values as $work_ShiftTypeWork_Shift_DataItem) {
            ...
            if (!$work_ShiftTypeWork_Shift_DataItem instanceof \WorkdayWsdl\\StructType\Work_Shift_DataType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Work_Shift_Data property can only contain items of type \WorkdayWsdl\\StructType\Work_Shift_DataType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @throws \InvalidArgumentException
     * @param \WorkdayWsdl\\StructType\Work_Shift_DataType $item
     * @return \WorkdayWsdl\\StructType\Work_ShiftType
     */
    public function addToWork_Shift_Data(\WorkdayWsdl\\StructType\Work_Shift_DataType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \WorkdayWsdl\\StructType\Work_Shift_DataType) {
            throw new \InvalidArgumentException(sprintf('The Work_Shift_Data property can only contain items of type \WorkdayWsdl\\StructType\Work_Shift_DataType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
