...
class Worker_Insurance_Period_DataType extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \StructType\Worker_Insurance_Coverage_DataType[]
     */
     ...
     * @param \StructType\Worker_Insurance_Coverage_DataType[] $insurance_Coverage_Data
     */
    public function __construct(\StructType\Enrollment_Period_DataType $enrollment_Period_Data = null, array $insurance_Coverage_Data = array())
    {
        $this
     ...
     * @param \StructType\Enrollment_Period_DataType $enrollment_Period_Data
     * @return \StructType\Worker_Insurance_Period_DataType
     */
     ...
     * @return \StructType\Worker_Insurance_Coverage_DataType[]|null
     */
    ...
    public static function validateInsurance_Coverage_DataForArrayConstraintsFromSetInsurance_Coverage_Data(array $values = array())
    {
        ...
        foreach ($values as $worker_Insurance_Period_DataTypeInsurance_Coverage_DataItem) {
            ...
            if (!$worker_Insurance_Period_DataTypeInsurance_Coverage_DataItem instanceof \StructType\Worker_Insurance_Coverage_DataType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Insurance_Coverage_Data property can only contain items of type \StructType\Worker_Insurance_Coverage_DataType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @throws \InvalidArgumentException
     * @param \StructType\Worker_Insurance_Coverage_DataType $item
     * @return \StructType\Worker_Insurance_Period_DataType
     */
    public function addToInsurance_Coverage_Data(\StructType\Worker_Insurance_Coverage_DataType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\Worker_Insurance_Coverage_DataType) {
            throw new \InvalidArgumentException(sprintf('The Insurance_Coverage_Data property can only contain items of type \StructType\Worker_Insurance_Coverage_DataType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
