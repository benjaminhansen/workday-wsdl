...
class Job_Family_Group_Response_DataType extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \StructType\Job_Family_Group_NewType[]
     */
     ...
     * @param \StructType\Job_Family_Group_NewType[] $job_Family_Group
     */
     ...
     * @return \StructType\Job_Family_Group_NewType[]|null
     */
    ...
    public static function validateJob_Family_GroupForArrayConstraintsFromSetJob_Family_Group(array $values = array())
    {
        ...
        foreach ($values as $job_Family_Group_Response_DataTypeJob_Family_GroupItem) {
            ...
            if (!$job_Family_Group_Response_DataTypeJob_Family_GroupItem instanceof \StructType\Job_Family_Group_NewType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Job_Family_Group property can only contain items of type \StructType\Job_Family_Group_NewType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @param \StructType\Job_Family_Group_NewType[] $job_Family_Group
     * @return \StructType\Job_Family_Group_Response_DataType
     */
     ...
     * @throws \InvalidArgumentException
     * @param \StructType\Job_Family_Group_NewType $item
     * @return \StructType\Job_Family_Group_Response_DataType
     */
    public function addToJob_Family_Group(\StructType\Job_Family_Group_NewType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\Job_Family_Group_NewType) {
            throw new \InvalidArgumentException(sprintf('The Job_Family_Group property can only contain items of type \StructType\Job_Family_Group_NewType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
