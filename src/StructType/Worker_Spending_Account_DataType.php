...
class Worker_Spending_Account_DataType extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \StructType\Worker_Spending_Account_Period_DataType[]
     */
     ...
     * @param \StructType\Worker_Spending_Account_Period_DataType[] $spending_Account_Period_Data
     */
     ...
     * @return \StructType\Worker_Spending_Account_Period_DataType[]|null
     */
    ...
    public static function validateSpending_Account_Period_DataForArrayConstraintsFromSetSpending_Account_Period_Data(array $values = array())
    {
        ...
        foreach ($values as $worker_Spending_Account_DataTypeSpending_Account_Period_DataItem) {
            ...
            if (!$worker_Spending_Account_DataTypeSpending_Account_Period_DataItem instanceof \StructType\Worker_Spending_Account_Period_DataType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Spending_Account_Period_Data property can only contain items of type \StructType\Worker_Spending_Account_Period_DataType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
     ...
     * @param \StructType\Worker_Spending_Account_Period_DataType[] $spending_Account_Period_Data
     * @return \StructType\Worker_Spending_Account_DataType
     */
     ...
     * @throws \InvalidArgumentException
     * @param \StructType\Worker_Spending_Account_Period_DataType $item
     * @return \StructType\Worker_Spending_Account_DataType
     */
    public function addToSpending_Account_Period_Data(\StructType\Worker_Spending_Account_Period_DataType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\Worker_Spending_Account_Period_DataType) {
            throw new \InvalidArgumentException(sprintf('The Spending_Account_Period_Data property can only contain items of type \StructType\Worker_Spending_Account_Period_DataType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
